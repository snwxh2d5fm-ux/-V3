...
- **代码**:
  ...
  for (var si = 0; si < allStages.length; si++) {
    if (allStages[si].status === 'in_progress') {
      ...
      else if (pid.includes('phase2') || pid.includes('onboarding')) currentStepIdx = 2;  // ← 硬编码2
...
```javascript
var hasAnyProgress = allStages.some(function(s) { return s.status === 'in_progress' || s.status === 'completed'; });
if (!hasAnyProgress) currentStepIdx = 1; // 全新流程
...
- **修复**: phases 构建时增加对已完成 stage 的二次校验:
  ...
  var allStepStagesDone = stepStages.length > 0 && stepStages.every(function(s) { return s.status === 'completed'; });
