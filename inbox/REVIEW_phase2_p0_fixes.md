...
**修复**:
1. success/fail 回调中调用 `wx.cloud.callFunction({name:'payment',data:{action:'confirmPayment',orderId}})` 
2. `payment.confirmPayment` 增加 `guidebook_unlock` 处理: `if (order.category === 'guidebook_unlock') { update users set guidebookAllUnlocked=true }`
3. 同样修 `identityReset` 支付流程
