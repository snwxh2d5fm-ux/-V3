const corpus = require('../../data/school-corpus.js');

const TYPE_MAP = { 官立: 'gov', 资助: 'aid', 直资: 'dss', 私立: 'private', 国际: 'intl' };
const NET_MAP = {
  名校网: ['11', '12', '34', '41'],
...
Page({
  ...
  doFilter: function () {
    ...
    const region = this.data.activeRegion;
    const fee = this.data.activeFee;
    const net = this.data.activeNet;
    const kw = (this.data.keyword || '').toLowerCase();
    const netNums = NET_MAP[net] || [];

    const filtered = this.data.schools.filter(function (s) {
      if (type !== '全部' && s.type !== type) return false;
      if (region && region !== '全部') {
        if (s.region.indexOf(region) < 0) return false;
      }
      if (fee && fee !== '全部') {
        const isFree = !s.fee || s.fee.indexOf('免费') >= 0 || s.fee.indexOf('HK$0') >= 0;
        if (fee === '免费' && !isFree) return false;
        if (fee === '收费' && isFree) return false;
      }
      if (netNums.length) {
        let found = false;
        for (let i = 0; i < netNums.length; i++) {
          if (s.net.indexOf(netNums[i]) >= 0) {
            found = true;
            break;
          }
        }
      ...
      if (kw) {
        const txt = (s.name + s.region + s.net + s.curriculum).toLowerCase();
