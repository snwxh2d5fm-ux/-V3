...
Page({
  ...
  loadProcess(processId) {
    ...
    const template = templates.processTemplates.find(function (t) {
      return t.id === process.templateId;
    });
    // 计算进度
    const completedStages = process.stages.filter(function (s) {
      return s.status === 'completed';
    }).length;
    const progress = Math.round((completedStages / process.stages.length) * 100);
    const updated = {};
    for (const k in process) {
      if (process.hasOwnProperty(k)) updated[k] = process[k];
    }
  ...
  toggleStage(e) {
    ...
    const stage = this.data.process.stages.find(function (s) {
      return s.id === stageId;
    });
  ...
  markStepDone(e) {
    try {
      ...
      let process = null;
      try {
        process = JSON.parse(JSON.stringify(this.data.process));
      } catch (e2) {
        return;
      }
      if (!process || !process.stages) return;
      const stage = process.stages.find(function (s) {
        return s.id === stageId;
      });
      if (!stage) return;
