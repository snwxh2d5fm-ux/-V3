...
export function ContentPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-xl font-bold">内容运营</h1><p className="text-sm text-[var(--muted-foreground)]">攻略书排行 · 任务完成率 · 搜索热词</p></div>

      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          <h2 className="mb-3 text-sm font-semibold">攻略书文章</h2>
          {articles.length === 0 ? <div className="text-xs text-[var(--muted-foreground)]">暂无文章</div> : (
            <div className="space-y-1 max-h-64 overflow-y-auto text-xs">
              {(articles as any[]).map((a, i) => (
                <div key={i} className="flex justify-between py-1 border-b border-[var(--border)] last:border-0">
                  <span>{a.title || a._id}</span><span className="text-[var(--muted-foreground)]">{a.category || ''}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        {hotwords.length === 0 ? <div className="text-xs text-[var(--muted-foreground)]">暂无数据</div> : (
