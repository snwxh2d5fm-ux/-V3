...
export function ShareAnalyticsPage() {
  ...
  useEffect(() => {
    let c = false;
    setLoading(true);
    Promise.all([callAdminFunction('/admin-codes', { action: 'getCodeStats', params: { codeType: 'invite' } })]).then(
      ([cs]) => {
        if (c) return;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">分享裂变分析</h1>
        <p className="text-sm text-[var(--muted-foreground)]">分享卡片 · 打开转化 · 邀请码激活 · 裂变链路</p>
      </div>
        ...
        <div className="space-y-3">
          {[
            { step: '生成分享卡片', count: totalShares, pct: 100, color: 'bg-[var(--primary)]' },
            {
              step: '好友打开卡片',
              count: totalOpens,
              pct: Math.round((totalOpens / totalShares) * 100),
              color: 'bg-blue-500',
            },
            ...
            {
              step: '激活邀请码',
              count: (codeStats.activated as number) || 0,
              pct: Math.round((((codeStats.activated as number) || 0) / totalShares) * 100),
              color: 'bg-red-500',
            },
          ].map((s, i) => (
      ...
      <div className="rounded-lg border border-[var(--muted)] bg-[var(--muted)] p-4">
        ...
        <ul className="space-y-1 text-xs text-[var(--secondary-foreground)]">
          ...
          <li>
            邀请码生成 {String(codeStats.generated || 0)} 张、激活 {String(codeStats.activated || 0)} 张，激活率{' '}
            {String(codeStats.activationRate || '0%')}
          </li>
