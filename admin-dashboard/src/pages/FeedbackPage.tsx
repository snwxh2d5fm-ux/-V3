...
const TYPE_LABELS: Record<string, string> = {
  bug: '功能异常', feature: '功能建议', content: '内容错误',
  usability: '体验问题', speed: '速度投诉', code: '内测码相关'
};
...
export function FeedbackPage() {
  ...
  useEffect(() => { loadData(typeFilter); }, [typeFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        <h2 className="mb-3 text-sm font-semibold">反馈列表 ({typeFilter === '全部' ? '全部' : TYPE_LABELS[typeFilter] || typeFilter})</h2>
        {loading ? <div className="py-8 text-center text-sm text-[var(--muted-foreground)]">加载中...</div> :
        feedback.length === 0 ? <div className="py-8 text-center text-sm text-[var(--muted-foreground)]">暂无反馈</div> : (
              ...
              <div key={i} className="rounded border border-[var(--border)] p-3 text-xs">
                <div className="flex justify-between mb-1 flex-wrap gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded bg-[var(--muted)] px-1.5 py-0.5">{TYPE_LABELS[f.type] || f.type || '未知'}</span>
                  </div>
                  <span className={`rounded px-1.5 py-0.5 ${f.status === 'resolved' ? 'bg-green-950 text-green-400' : f.status === 'in_progress' ? 'bg-blue-950 text-blue-400' : 'bg-yellow-950 text-yellow-400'}`}>
