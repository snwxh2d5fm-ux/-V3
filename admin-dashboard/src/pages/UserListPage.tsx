...
interface EnrichedUser {
  _openid: string; persona: number; personaLabel: string;
  selectedPath: string; pathLabel: string;
  membershipTier: string; userStatus: string;
  isLocked: boolean; onboardingCompleted: boolean;
  switchCount: number; lastActiveAt: number; createdAt: number;
}
...
const PATH_LABELS: Record<string, string> = {
  qmas: '优才', ttps_b: '高才B', ttps_a: '高才A', asmtp: '专才',
  iang: 'IANG', student_iang: '升学', dependent: '受养人', renewal: '续签', pr: '永居'
};
...
export function UserListPage() {
  ...
  useEffect(() => { loadStats(); }, []);
  useEffect(() => { loadUsers(); }, [personaFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[{ title: '身份分布', data: stats.byPersona }, { title: '路径分布', data: stats.byPath, labels: PATH_LABELS }, { title: '会员分布', data: stats.byMembership }, { title: '状态分布', data: stats.byStatus }].map((group, i) => (
      ...
      <div className="flex gap-1 rounded-lg bg-[var(--muted)] p-1 flex-wrap">
        {PERSONA_FILTERS.map(f => (
          <button key={f} onClick={() => setPersonaFilter(f)}
            className={`rounded-md px-3 py-1 text-xs transition-colors ${personaFilter === f ? 'bg-[var(--card)] text-[var(--foreground)]' : 'text-[var(--muted-foreground)]'}`}>{f}</button>
        ))}
      </div>
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] overflow-x-auto">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-[var(--border)] text-left text-[var(--muted-foreground)]">
              <th className="px-3 py-2">用户</th><th className="px-3 py-2">身份</th><th className="px-3 py-2">路径</th><th className="px-3 py-2">会员</th><th className="px-3 py-2">状态</th><th className="px-3 py-2">切换</th><th className="px-3 py-2">最近活跃</th>
            </tr></thead>
            <tbody>
              {users.map((u, i) => {
                ...
                <tr key={i} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--accent)]">
                  ...
                  <td className="px-3 py-2"><span className="rounded bg-[var(--muted)] px-1.5 py-0.5">{u.personaLabel}</span></td>
                  ...
                  <td className="px-3 py-2"><span className={`rounded px-1.5 py-0.5 ${stColor === 'green' ? 'bg-green-950 text-green-400' : stColor === 'blue' ? 'bg-blue-950 text-blue-400' : stColor === 'yellow' ? 'bg-yellow-950 text-yellow-400' : 'bg-[var(--muted)] text-[var(--muted-foreground)]'}`}>{u.userStatus}</span></td>
