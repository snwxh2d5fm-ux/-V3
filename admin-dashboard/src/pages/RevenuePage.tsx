...
export function RevenuePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">财务看板</h1>
        <p className="text-sm text-[var(--muted-foreground)]">收入概览 · 订单管理 · 开票申请</p>
      </div>
      ...
      <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        {orders.length === 0 ? (
          <div className="text-xs text-[var(--muted-foreground)]">暂无订单</div>
        ) : (
            ...
            <tbody>
              {(orders as any[]).map((o, i) => (
                <tr key={i} className="border-b border-[var(--border)] last:border-0">
                  <td className="py-2">{o.productName || o._id}</td>
                  <td className="py-2">¥{((o.amount || 0) / 100).toFixed(0)}</td>
                  <td className="py-2">
                    <span
                      className={`rounded px-1.5 py-0.5 ${o.status === 'completed' ? 'bg-green-950 text-green-400' : 'bg-yellow-950 text-yellow-400'}`}
                    >
                      {o.status}
                    </span>
                  </td>
                  <td className="py-2">{String(o.createdAt || '').slice(0, 10)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        {invoices.length === 0 ? (
          <div className="text-xs text-[var(--muted-foreground)]">暂无开票申请</div>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-[var(--border)] text-left text-[var(--muted-foreground)]">
                <th className="pb-2">发票号</th>
                <th className="pb-2">类型</th>
                <th className="pb-2">抬头</th>
                <th className="pb-2">金额</th>
                <th className="pb-2">状态</th>
                <th className="pb-2">时间</th>
              </tr>
            ...
            <tbody>
              {(invoices as any[]).map((inv, i) => (
                <tr key={i} className="border-b border-[var(--border)] last:border-0">
                  ...
                  <td className="py-2">
                    <span
                      className={`rounded px-1.5 py-0.5 ${inv.status === 'issued' ? 'bg-green-950 text-green-400' : inv.status === 'rejected' ? 'bg-red-950 text-red-400' : 'bg-yellow-950 text-yellow-400'}`}
                    >
                      {inv.status === 'issued' ? '已开' : inv.status === 'rejected' ? '已驳' : '待开'}
                    </span>
                  </td>
