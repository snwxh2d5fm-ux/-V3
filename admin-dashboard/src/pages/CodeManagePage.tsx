  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">管理种子用户邀请码和付费会员兑换码</p>
      </div>
      {error && <div className="rounded-md bg-red-950/50 px-3 py-2 text-xs text-red-400">{error}</div>}
      <div className="grid grid-cols-3 gap-4">
        {[
          ['已生成', codeStats.generated as number],
          ['已激活', codeStats.activated as number],
          ['激活率', codeStats.activationRate as string],
        ].map(([l, v], i) => (
      ...
      <div className="flex gap-1 rounded-lg bg-[var(--muted)] p-1">
        {['invite', 'redemption'].map((t) => (
          <button
            key={t}
            data-testid={`tab-${t}`}
            onClick={() => setActiveTab(t as 'invite' | 'redemption')}
        ...
        <div className="mt-3 flex gap-2">
          ...
          {showConfirm && (
            <button
              ...
              className="rounded-md border border-[var(--border)] px-4 py-1.5 text-sm hover:bg-[var(--accent)]"
            >
              取消
            </button>
          )}
              ...
              <tbody>
                {codes.map((c, i) => (
                  <tr key={i} data-testid="code-row" className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="py-2">
                      {c.activationCount}/{c.maxActivations}
                    </td>
