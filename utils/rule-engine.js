...
function runMaterialCheck(processType, userDocIds) {
  ...
  const items = rules.map(rule => {
    ...
    return { id: rule.id, name: rule.name, requiredCategory: rule.requiredCategory, status, matchedDocId: matched ? matched.id : null, detail };
  });
  const ready = items.filter(i => i.status === 'ready').length;
  return {
    total: items.length, ready,
    missing: items.filter(i => i.status === 'missing').length,
    pending: items.filter(i => i.status === 'pending').length,
    expired: items.filter(i => i.status === 'expired').length,
    items, score: Math.round((ready / items.length) * 100)
  };
}
      ...
      fieldMappings.forEach(mapping => {
        mapping.fields.forEach(f1 => {
          mapping.fields.forEach(f2 => {
            ...
            if (v1 && v2 && v1 !== v2) {
              issues.push({
                severity: 'high', type: 'inconsistency', field: mapping.label,
...
function validateMilestone(targetStage, ocrResult) {
  ...
  for (const rule of validations) {
    ...
    if (rule.pattern && value) {
      ...
      if (!re.test(value)) {
        return { valid: false, reason: `字段 ${rule.field} 格式不符合预期: ${value}`, confidence: rule.confidence || 'A' };
      }
    }
  }
...
function checkPolicyVersion(text) {
  const deprecatedPolicies = [
    { pattern: /20(?:19|20|21|22)年.*学生.*工作.*(?:限制|小时|每周)/g, issue: '学生工作限制政策已于2023/2024年更新，旧政策已废止 [A]' },
...
function getBuiltinReminderRules() {
  return [
    // 通用: 获批后
    { rule_id: 'R_APPROVAL_001', path: 'all', phase: 'phase2_onboarding', milestone_tag: 'dp1_initial_path',
      trigger: { event: 'approval_received', date_field: 'approvalDate' }, confidence: 'A',
      legal_basis: 'Cap.115 签证条款', reminders: [
    ...
    // 学生签证激活后
    { rule_id: 'R_VISA_ACTIVATE_001', path: 'student_iang', phase: 'phase2_onboarding', milestone_tag: 'dp2_student_to_work',
      trigger: { event: 'visa_activated', date_field: 'activationDate' }, confidence: 'A',
      legal_basis: 'Cap.177 香港身份证', reminders: [
        { label: '申办香港身份证(30天内)', offset_days: 30, alerts: [21, 14, 7, 3, 1] }
      ] },
    // 学生签证到期前: 准备IANG
    { rule_id: 'R_STUDENT_IANG_001', path: 'student_iang', phase: 'phase3_maintenance', milestone_tag: 'dp2_student_to_work',
      trigger: { event: 'visa_expiring', date_field: 'expiryDate' }, confidence: 'A',
      legal_basis: 'IANG政策', reminders: [
        { label: 'IANG准备启动(毕业前3月)', offset_days: -90, alerts: [180, 120, 90, 60, 30] },
        { label: '凭Completion Letter申请IANG', offset_days: 0, alerts: [30, 14, 7] }
      ] },
    // 签证到期前: 续签通用
    { rule_id: 'R_RENEWAL_001', path: 'all', phase: 'phase3_maintenance', milestone_tag: 'dp3_renewal_strategy',
      trigger: { event: 'visa_expiring', date_field: 'expiryDate' }, confidence: 'A',
      legal_basis: 'Cap.115 签证续签', reminders: [
        { label: '续签材料准备启动', offset_days: -90, alerts: [180, 90, 60, 30] },
        { label: '递交续签申请', offset_days: 0, alerts: [90, 60, 30, 14, 7] }
      ] },
    // 永居冲刺
    { rule_id: 'R_PR_001', path: 'all', phase: 'phase4_pr_sprint', milestone_tag: 'dp5_pr_sprint',
      trigger: { event: 'pr_eligible', date_field: 'sevenYearDate' }, confidence: 'A',
      legal_basis: 'Cap.115 永居申请', reminders: [
    ...
    { rule_id: 'R_IANG_RENEWAL_001', path: 'student_iang', phase: 'phase3_maintenance', milestone_tag: 'dp3_renewal_strategy',
      trigger: { event: 'iang_24mo_expiring', date_field: 'expiryDate' }, confidence: 'B',
      legal_basis: 'IANG续签政策', reminders: [
        { label: '决策节点③续签策略: 雇主/自雇/创业', offset_days: -90, alerts: [90, 60, 30] },
        { label: '确认MPF+税单记录完整', offset_days: -60, alerts: [60, 30, 14] }
      ] },
    // 优才12准则自评
    { rule_id: 'R_QMAS_ASSESS_001', path: 'qmas', phase: 'phase1_evaluation', milestone_tag: 'dp1_initial_path',
      trigger: { event: 'qmas_assessment_started', date_field: 'startDate' }, confidence: 'C',
      legal_basis: '优才新制12项准则', reminders: [
        { label: '完成12项准则逐项评估', offset_days: 14, alerts: [7, 3, 1] },
        { label: '确认满足≥6项准则', offset_days: 14, alerts: [3, 1] }
      ] },
    // 高才B续签(24月后)
    { rule_id: 'R_TTPS_B_RENEWAL_001', path: 'ttps_b', phase: 'phase3_maintenance', milestone_tag: 'dp3_renewal_strategy',
      trigger: { event: 'ttps_24mo_expiring', date_field: 'expiryDate' }, confidence: 'B',
      legal_basis: '高才通续签指引', reminders: [
        { label: '确认在港就业/创业证据完整', offset_days: -90, alerts: [90, 60, 30, 14] }
      ] }
  ];
}
...
function getBuiltinCheckRules() {
  return [
    ...
    { id: 'CHK_STUDENT_03', processType: 'student_iang', name: 'Completion Letter(IANG用)', requiredCategory: 'education' },
...
function getBuiltinValidationRules() {
  return [
    { stage: 'hk_id_application', field: 'hkIdNumber', required: true, pattern: '[A-Z]{1,2}\\d{6,7}\\([0-9A]\\)', confidence: 'A' }
  ];
}

function getBuiltinLegalRules() {
  return [
    { id: 'LEGAL_CHECK_001', type: 'cap115_2A_to_11_8', pattern: /s\.?2A/gi, severity: 'P0',
      fix: 'Cap.115 s.11(8) (入境处处长酌情权)', reason: 's.2A定义居留权，非"视为合法逗留"机制' },
    { id: 'LEGAL_CHECK_002', type: 'cap115_42_to_38A', pattern: /s\.?42(?!\s*被)/gi, severity: 'P0',
      fix: 'Cap.115 s.38A (虚假陈述)', reason: '虚假陈述条文为s.38A' }
  ];
}

module.exports = {
  loadRules, matchReminderRules, runMaterialCheck, runConsistencyCheck, validateMilestone,
  parseDate, expandReminders,
  // V5新增
  checkLegalCitation, checkAbsoluteLanguage, checkPolicyVersion,
  P0_LEGAL_FIXES, P0_POLICY_FIXES
};
