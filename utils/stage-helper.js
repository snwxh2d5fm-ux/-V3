...
function getActiveStageIndex() {
  try {
    ...
    const ps = wx.getStorageSync('__process_stage__');
    ...
    if (app && app.globalData && app.globalData.userStatus) {
      const s = app.globalData.userStatus;
    ...
    if (app && app.globalData && app.globalData.activeProcess) {
      const ap = app.globalData.activeProcess;
    ...
    // 5. 其他storage检查
    const userStatus = wx.getStorageSync('_user_status') || wx.getStorageSync('userStatus') || '';
    if (userStatus === 'approved') return 3;
    if (userStatus === 'submitted') return 2;
    if (userStatus === 'unapplied') return 0;

    const path = wx.getStorageSync('__selected_path__') || wx.getStorageSync('__active_process_id__') || '';
    if (path) return 1;
  } catch (e) {
