...
async function seedFromFixture(mp, fixturePath, storageKey) {
  await mp.evaluate(
    function (fp, key) {
      ...
      const raw = fsm.readFileSync(fp, 'utf8');
...
beforeEach(async () => {
  ...
  await mp.evaluate(function () {
    const keep = { auth_token: wx.getStorageSync('auth_token'), user_profile: wx.getStorageSync('user_profile') };
    wx.clearStorageSync();
  ...
  test('切换到配偶 → 溢出区无本人证件', async () => {
    ...
    const result = await mp.evaluate(function () {
      ...
      return {
        ...
        overflowOwners: overflowDocs.map(function (d) {
