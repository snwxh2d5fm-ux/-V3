...
describe('#1 长周期提醒已移除', () => {
  test('选择QMAS路径后不应弹出长周期提醒弹窗', async () => {
    ...
    // 通过 evaluate 检查弹窗状态
    const popupVisible = await mp.evaluate(function () {
      const page = getCurrentPages()[getCurrentPages().length - 1];
      return page && page.data ? !!page.data.showDisclaimerPopup : null;
    });
    // 如果有弹窗，不应是长周期类型
    if (popupVisible === true) {
      const popupType = await mp.evaluate(function () {
        const page = getCurrentPages()[getCurrentPages().length - 1];
...
describe('#4 提醒器时间线自动生成', () => {
  test('选QMAS路径后应自动生成时间线节点', async () => {
    ...
    const result = await mp.evaluate(function () {
      const page = getCurrentPages()[getCurrentPages().length - 1];
      if (!page || !page.data) return { stages: 0, path: '' };
      const stages = page.data.timelineStages || [];
...
describe('#5 攻略UGC内容非空白', () => {
  test('攻略详情页内容应非空（加载成功且无加载错误）', async () => {
    ...
    // 验证加载完成 + 无错误
    const loadState = await mp.evaluate(function () {
      const page = getCurrentPages()[getCurrentPages().length - 1];
    ...
    const hasAnyContent = loadState.hasGuide || loadState.parsedCount > 0;
...
describe('#7 配偶证件隔离', () => {
  test('切换身份到配偶 → 仅显示配偶证件', async () => {
    ...
    // 检查页面上显示的卡槽数据
    const displayedOwnerTypes = await mp.evaluate(function () {
      const page = getCurrentPages()[getCurrentPages().length - 1];
      if (!page || !page.data) return [];
      const slotCategories = page.data.slotCategories || [];
      // 检查溢出区文档
      const overflowDocs = page.data.overflowDocs || [];
      return overflowDocs.map(function (d) {
        return d.ownerType || 'self';
      });
    });
      ...
      wx.setStorageSync('__vault_meta__', {
        documents: {
          'test-doc-1': {
            ...
            docId: 'test-doc-1',
            docType: 'id_card',
            ocrData: { name: 'test', idNumber: 'test' },
            category: '身份文件',
          },
          'test-doc-2': {
            id: 'test-doc-2',
            docId: 'test-doc-2',
            docType: 'passport',
            ocrData: { name: 'test', passportNumber: 'test' },
            category: '身份文件',
          },
        },
        version: 1,
