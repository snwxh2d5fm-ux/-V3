...
describe('流程模板 — processTemplates 结构与完整性', function() {
  ...
  test('[P0] 模板ID唯一', function() {
    var ids = templates.processTemplates.map(function(t) { return t.id; });
    var uniqueIds = ids.filter(function(v, i, arr) { return arr.indexOf(v) === i; });
  ...
  test('[P1] 每个阶段至少有一个milestones或steps', function() {
    templates.processTemplates.forEach(function(tpl) {
      tpl.phases.forEach(function(phase) {
        var hasMilestones = Array.isArray(phase.milestones) && phase.milestones.length > 0;
        var hasSteps = Array.isArray(phase.steps) && phase.steps.length > 0;
...
describe('流程模板 — 关键路径验证', function() {
  ...
  test('[P0] 学生IANG模板存在', function() {
    var tpl = templates.processTemplates.find(function(t) {
  ...
  test('[P1] 高才A模板riskLevel验证', function() {
    var tpl = templates.processTemplates.find(function(t) { return t.id === 'ttps_a'; });
    expect(tpl.riskLevel).toBeDefined();
  });
});
