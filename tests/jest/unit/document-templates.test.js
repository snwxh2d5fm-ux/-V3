...
const docTemplates = require('../../../data/document-index-templates');

const VALID_STATUSES = ['unapplied', 'applied', 'approved', 'renewal', 'permanent', 'any'];
const VALID_PATHS = [
  'qmas',
  'ttps_a',
  'ttps_b',
...
const VALID_MODES = ['application', 'renewal', 'conversion'];
const VALID_REQUIREMENTS = ['required', 'recommended', 'optional'];
...
describe('证件索引模板 — INDEX_TEMPLATES 结构', function () {
  test('[P0] INDEX_TEMPLATES 存在且非空', function () {
    ...
    const keys = Object.keys(docTemplates.INDEX_TEMPLATES);
    expect(keys.length).toBeGreaterThanOrEqual(10);
  });

  test('[P0] 每个模板有 templateId/status/path/mode/totalRequired/categories', function () {
    const templates = docTemplates.INDEX_TEMPLATES;
  ...
  test('[P1] slotKey在模板内唯一 (允许 overflowZone 复用)', function () {
    ...
    Object.keys(templates).forEach(function (key) {
      ...
      const overLimit = Object.keys(counts).filter(function (k) {
        return counts[k] > 2;
      });
...
describe('证件索引模板 — matchTemplate()', function () {
  ...
  test('[P0] 有效三元组返回模板', function () {
    const result = docTemplates.matchTemplate('unapplied', 'qmas', 'application');
  ...
  test('[P1] 无效组合返回默认模板(不崩)', function () {
    const result = docTemplates.matchTemplate('nonexistent', 'qmas', 'application');
    expect(result).toBeDefined();
    if (result) {
  ...
  test('[P1] 每个已注册status+path+mode组合有匹配', function () {
    ...
    keys.forEach(function (key) {
      const tpl = templates[key];
      const result = docTemplates.matchTemplate(tpl.status, tpl.path, tpl.mode);
      expect(result).toBeDefined();
...
describe('证件索引模板 — computeSlotStates()', function () {
  ...
  test('[P0] 传入模板+空数组 → 返回含填充状态的分类', function () {
    const template = docTemplates.matchTemplate('unapplied', 'qmas', 'application');
    const result = docTemplates.computeSlotStates(template, []);
    expect(result).toBeDefined();
  ...
  test('[P1] 空模板返回空数组(期待不崩)', function () {
    try {
      const result = docTemplates.computeSlotStates(null, []);
      expect(Array.isArray(result)).toBe(true);
    } catch (e) {
