...
const r1 = assemblePath(P.fresh_single);
assert(r1.tasks.length > 0);
assert(r1.phases.length > 0);
console.log('   fresh/single:', r1.tasks.length, '任务', r1.phases.length, '关');

const r2 = assemblePath(P.pre_single);
assert(r2.tasks.length > 0);
console.log('   pre/single: ', r2.tasks.length, '任务（仅关卡0）');

const r3 = assemblePath(P.fresh_preschool);
const hasP5 = r3.tasks.some(function (t) {
  return t.phase === 5;
});
assert(hasP5);
...
const lr = cache.fetchByPathLocal('ttps-bc', 'single', 'fresh', []);
...
checks.forEach(function (c) {
  const tasks = assemblePath(c.p).tasks;
  const t = tasks.find(function (x) {
    return x.id === c.id;
  });
  if (!t) {
    console.log('   ' + c.id + ' MISSING — FAIL');
    process.exit(1);
  }
  if (!t.steps || t.steps.length < c.min) {
    console.log('   ' + c.id + ' 步骤不足 — FAIL');
    process.exit(1);
  }
  let truncated = false;
  t.steps.forEach(function (s, i) {
    if (!s.content || s.content.length < 10) truncated = true;
  });
  if (truncated) {
    console.log('   ' + c.id + ' 内容截断 — FAIL');
    process.exit(1);
  }
...
const raw = assemblePath(P.pre_single).tasks.find(function (t) {
  return t.id === 'onboard-003';
});
const n = norm(raw, {});
assert(n._id === 'onboard-003');
assert(n.id === 'onboard-003');
...
const mockCB = [
  {
    ...
    phase: 3,
    urgency: '必修',
    steps: [{ seq: 1, title: '?', content: '截断...' }],
  },
];
const local300 = assemblePath(P.fresh_single).tasks.filter(function (t) {
  return t.id === 'onboard-300';
});
const merged = merge(local300, mockCB);
const m300 = merged.find(function (t) {
  return t.title === '完成找房向导';
});
...
const lr2 = cache.fetchByPathLocal('ttps-bc', 'single', 'fresh', []);
const tasks = lr2.data.tasks.map(function (t) {
  return norm(t, {});
});
...
function merge(localTasks, cloudTasks) {
  ...
  cloudTasks.forEach(function (ct) {
    ...
    if (lm) {
      const m = JSON.parse(JSON.stringify(lm));
      if (ct._id) m._id = ct._id;
      out.push(m);
      delete idx[ct.title];
    } else out.push(ct);
  });
