...
Page({
  ...
  onSelect(e) {
    ...
    const opts = this.data.pathOptions;
    let label = '';
    ...
    Promise.race([
      ...
      .then(function (startRes) {
        if (startRes && startRes.result && startRes.result.code === 0 && startRes.result.data) {
          const cloudId = startRes.result.data.processId;
          const savedProcessId = processLine.id;
          const lines = getAllProcessLines();
          const line = lines.find(function (l) {
            return l.id === savedProcessId;
          });
          if (line) {
  ...
  onGatePassed: function () {
    ...
    const label = this.data.pendingPathLabel;
