...
function housingWizard(page, storage) {
  return {
    ...
    onWizardNext: function (e) {
      const step = page.data.wizardStep;
      ...
      if (step === 0) {
        ...
        const budgetId = page.data.wizardBudget;
        let budgetValue = 10000;
        const brackets = districtData.BUDGET_BRACKETS;
        for (let bi = 0; bi < brackets.length; bi++) {
          if (brackets[bi].id === budgetId) {
            budgetValue = brackets[bi].min;
            break;
          }
        }
        const results = districtData.matchDistricts(budgetValue, page.data.wizardWork, page.data.wizardHasKids);
        const COMMUTE_MAP = {
          central: 'central',
          wanchai: 'causewayBay',
        ...
        const subKey = page.data.wizardSubRegion;
        results.forEach(function (r) {
          r.stars = r.familyFriendly ? new Array(r.familyFriendly + 1).join('⭐') : '⭐';
          r._hasSchoolNet = !!(r.schoolNet && r.schoolNet.primary > 0);
          const commuteField = COMMUTE_MAP[subKey] || 'central';
          r._commuteLabel =
            (SUB_COMMUTE[subKey] || '中环') + ' ' + (r.commute[commuteField] || r.commute.central) + 'min';
        });
    ...
    onWizardDone: function () {
      ...
      const phases = page.data.phases.map(function (p) {
    ...
    onWizardClose: function () {
      page.setData({ showHousingWizard: false });
    },
  };
}
...
function schoolNetWizard(page) {
  return {
    ...
    onSNWizardNext: function (e) {
      const step = page.data.snWizardStep;
      const value = e.currentTarget.dataset.value;
      ...
      if (step === 2) {
        const schoolNetData = require('../../../data/school-net-data');
        const results = schoolNetData.matchSchoolNets(
          page.data.snWizardLevel,
          page.data.snWizardRegion,
          page.data.snWizardBudget,
        );
        results.forEach(function (r) {
        ...
        page.setData({ snWizardStep: step + 1 });
      }
    },
