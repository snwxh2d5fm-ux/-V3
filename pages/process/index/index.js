...
Page({
  ...
  onSelectDirectPath(e) {
    ...
    const opts = this.data.directPathOptions;
    let label = '';
    let desc = '';
    ...
    const that = this;
    setTimeout(function () {
      that.__selectingPath = false;
    }, 2000);
    // ★ 封存旧路径提醒
    const oldPath = app.globalData.selectedPath || wx.getStorageSync('__selected_path__') || '';
    ...
    // 清除旧流程残留，防止阶段跳转
    const oldLines = getAllProcessLines();
    ...
    const tmpl = templates.processTemplates.find(function (t) {
      return t.id === id || t.pathType === id;
    });
    const stages = [];
    if (tmpl && tmpl.phases) {
      tmpl.phases.forEach(function (p) {
        ...
        const stageSteps = (p.steps || []).map(function (st) {
    ...
    const processLine = {
      ...
      pathType: id,
      riskLevel:
        constants.PATH_RISK_LEVELS && constants.PATH_RISK_LEVELS[id] ? constants.PATH_RISK_LEVELS[id].level : 'medium',
    ...
    const cloudProcessId = processLine.id;
    const cloudTimeout = new Promise(function (_, reject) {
      setTimeout(function () {
    ...
    Promise.race([
      ...
      .then(function (startRes) {
        if (
          startRes &&
          startRes.result &&
          startRes.result.code === 0 &&
          startRes.result.data &&
          startRes.result.data.processId
        ) {
          ...
          if (line) {
            line.cloudId = startRes.result.data.processId;
            saveProcessLine(line);
          }
        }
      })
      .catch(function (e) {
  ...
  completeAllSteps: async function (e) {
    ...
    const phaseId = BRIDGE.ui_to_phase[index];
    let currentStage = allStages.find(function (s) {
      return (s.phaseId && s.phaseId === phaseId) || (s.stageId && s.stageId === phaseId);
    });
    if (!currentStage && index < allStages.length) {
      const sortedStages = allStages.slice().sort(function (a, b) {
    ...
    const stageId = currentStage.stageId || currentStage.id;
    const pendingSteps = (currentStage.steps || []).filter(function (st) {
    ...
    const processId = this.data.activeProcessId || wx.getStorageSync('__active_process_id__');
    if (!processId) {
    ...
    processStep(0)
      .then(function () {
        ...
        if (lastResult && lastResult.result && lastResult.result.code === 0) {
          ...
          const errMsg = (lastResult && lastResult.result && lastResult.result.msg) || '操作失败';
  ...
  _localAdvanceStage: function (stageIdx) {
    try {
      const localId = this.data.activeProcessId || wx.getStorageSync('__active_process_id__') || '';
      const { getAllProcessLines: gl, saveProcessLine: sp } = require('../../../utils/storage');
      const ls = gl();
      const line = ls.find(function (l) {
        return l.id === localId || l.cloudId === localId;
      });
      if (line && line.stages) {
        let doneIdx = -1;
      ...
      console.warn('[_localAdvanceStage] 失败:', e);
    }
  },

  // ★ 通道B: 上传里程碑材料验证
  uploadMilestone: function (e) {
    const index = e.currentTarget.dataset.stageIndex;
    const phase = this.data.phases[index];
    if (!phase || phase.status !== 'current') return;
    const localProcessId = this.data.activeProcessId || wx.getStorageSync('__active_process_id__') || '';
    // 优先使用云端processId（verifyMilestone查询云端user_processes集合）
    let cloudProcessId = '';
    try {
      const lines = getAllProcessLines();
      var currentLine = lines.find(function (l) {
        return l.id === localProcessId || l.cloudId === localProcessId;
      });
      cloudProcessId = (currentLine && currentLine.cloudId) || '';
    } catch (e) {}
    const processId = cloudProcessId || localProcessId;
    // ★ 从本地流程线中查找实际的stageId
    let actualStageId = phase.stageId || phase.id;
    if (currentLine && currentLine.stages) {
      const inProgressStage = currentLine.stages.find(function (s) {
    ...
    wx.navigateTo({
      url:
        ...
        (phase.id || '') +
        '&milestoneType=' +
        (phase.milestoneDocType || '') +
        '&label=' +
        encodeURIComponent(phase.name || ''),
    });
  },
  ...
  loadActiveProcess() {
    try {
      ...
      const _toStepIdx = function (pid, order) {
        if (pid.includes('phase1') || pid.includes('evaluation')) return 0; // → 资格评估
        if (pid.includes('phase2') || pid.includes('onboarding')) {
          const total = allStages.filter(function (ss) {
            return (ss.phaseId || '').includes('phase2') || (ss.phaseId || '').includes('onboarding');
          }).length;
          let localOrder = (order || 0) - _phase2MinOrder;
      ...
      const doneCount = allStages.filter(function (s) {
      ...
      for (let si = 0; si < allStages.length; si++) {
        if (allStages[si].status === 'in_progress') {
          let mapped = _toStepIdx(allStages[si].phaseId || '', allStages[si].order);
          // P0-02: mapped=-1表示未知phaseId，按order兜底计算避免回退到步骤1
          if (mapped === -1) {
            // 兜底: 按全局order比例映射到7步
            const ratio = allStages.length > 1 ? si / (allStages.length - 1) : 0;
      ...
      if (currentStepIdx === 0) {
        if (allStages.length > 0 && doneCount === allStages.length) {
          ...
          for (let sj = allStages.length - 1; sj >= 0; sj--) {
            if (allStages[sj].status === 'completed') {
              const cm = _toStepIdx(allStages[sj].phaseId || '', allStages[sj].order);
              if (cm >= 1) {
                lastCompletedIdx = cm;
                break;
              }
      ...
      const phases = SEVEN_STEPS.map((step, i) => {
        ...
        return {
          ...
          stepIndex: i,
          status,
          isMilestone: constants.STAGE_BRIDGE_MAP.ui_stages[i]
            ? constants.STAGE_BRIDGE_MAP.ui_stages[i].isMilestone
            : false,
          milestoneDocType: constants.STAGE_BRIDGE_MAP.ui_stages[i]
            ? constants.STAGE_BRIDGE_MAP.ui_stages[i].milestoneDocType
            : null,
          materials: mats,
  ...
  toggleTemplateSelect() {
    this.setData({ showTemplateSelect: !this.data.showTemplateSelect });
  },
  catchStop() {},
  closeDisclaimer() {
  ...
  selectTemplate(e) {
    ...
    // ★ 封存旧路径提醒
    const oldPath = app.globalData.selectedPath || wx.getStorageSync('__selected_path__') || '';
    const newPath = template.pathType || templateId;
  ...
  onGatePassed: function () {
    const id = this.data.pendingPathId;
    const templateId = this.data.pendingTemplateId;
    this.setData({
      showGateSheet: false,
      gateMode: '',
      pendingPathId: '',
