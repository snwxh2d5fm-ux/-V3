...
var REMINDER_KEY = storage.REMINDER_KEY;
...
function seedReminders(entries) {
  global.__mockStorage[REMINDER_KEY] = entries.map(function(e) {
    return { id: e.id || 'r_' + Math.random().toString(36).slice(2,8),
             path: e.path, status: e.status, title: e.title || '' };
  });
}

function getReminders() { return global.__mockStorage[REMINDER_KEY] || []; }

describe('archiveRemindersByPath (P0修复)', function() {
  beforeEach(function() { clearMockStorage(); });

  test('空 pathId → 不操作，不报错', function() {
    seedReminders([{ path: 'qmas', status: 'active' }]);
    expect(function() { storage.archiveRemindersByPath(''); }).not.toThrow();
    expect(function() { storage.archiveRemindersByPath(null); }).not.toThrow();
    expect(function() { storage.archiveRemindersByPath(undefined); }).not.toThrow();
    // 数据不变
  ...
  test('空提醒列表 → 不报错', function() {
    seedReminders([]);
    expect(function() { storage.archiveRemindersByPath('qmas'); }).not.toThrow();
...
describe('unarchiveRemindersByPath (P0修复)', function() {
  ...
  test('空 pathId → 不操作，不报错', function() {
    ...
    expect(function() { storage.unarchiveRemindersByPath(''); }).not.toThrow();
    expect(function() { storage.unarchiveRemindersByPath(null); }).not.toThrow();
    expect(function() { storage.unarchiveRemindersByPath(undefined); }).not.toThrow();
    expect(getReminders()[0].status).toBe('archived');
  });
  ...
  test('空提醒列表 → 不报错', function() {
    seedReminders([]);
    expect(function() { storage.unarchiveRemindersByPath('qmas'); }).not.toThrow();
  });

  test('只有变化时才写 Storage（性能优化验证）', function() {
    seedReminders([{ path: 'qmas', status: 'active' }]);
    var before = JSON.stringify(getReminders());
    storage.unarchiveRemindersByPath('qmas');
    var after = JSON.stringify(getReminders());
    // 无封存提醒待恢复 → 数据应不变
...
describe('archiveRemindersByPath + unarchiveRemindersByPath 往返', function() {
  ...
  test('封存 → 恢复 → 状态回到 active', function() {
    ...
    storage.archiveRemindersByPath('qmas');
    expect(getReminders().every(function(r) { return r.status === 'archived'; })).toBe(true);

    storage.unarchiveRemindersByPath('qmas');
    expect(getReminders().every(function(r) { return r.status === 'active'; })).toBe(true);
    expect(getReminders().every(function(r) { return r.unarchivedAt; })).toBe(true);
  });
  ...
  test('跨路径操作互不干扰', function() {
    ...
    var afterArchive = getReminders();
    expect(afterArchive.filter(function(r) { return r.status === 'archived'; }).length).toBe(3);
    expect(afterArchive.find(function(r) { return r.path === 'asmpt'; }).status).toBe('active');

    // 恢复 qmas
    storage.unarchiveRemindersByPath('qmas');
    var afterUnarchive = getReminders();
    expect(afterUnarchive.filter(function(r) { return r.path === 'qmas' && r.status === 'active'; }).length).toBe(2);
    expect(afterUnarchive.find(function(r) { return r.path === 'ttps_a'; }).status).toBe('archived');
    expect(afterUnarchive.find(function(r) { return r.path === 'asmpt'; }).status).toBe('active');
  });
});
