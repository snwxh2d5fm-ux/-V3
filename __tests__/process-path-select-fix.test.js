...
describe('onSelectDirectPath — 集成测试 (P0)', function () {
  ...
  test('无旧路径时 archiveRemindersByPath 不抛异常', function () {
    ...
    const oldPath = global.getApp().globalData.selectedPath || global.__mockStorage['__selected_path__'] || '';
    expect(oldPath).toBe('');
  ...
  test('有旧路径切换时 archiveRemindersByPath 正常执行', function () {
    const st = require('../utils/storage');
    const REMINDER_KEY = st.REMINDER_KEY;

    // 模拟旧路径 'ttps_a' 的活跃提醒
    global.__mockStorage[REMINDER_KEY] = [{ id: 'r1', path: 'ttps_a', status: 'active', title: '高才通提醒' }];
    ...
    const reminders = global.__mockStorage[REMINDER_KEY];
  ...
  test('unarchiveRemindersByPath 直接调用不抛异常（核心修复）', function () {
    ...
    const r = global.__mockStorage[st.REMINDER_KEY];
  ...
  test('onSelectDirectPath 对有效路径ID的完整调用链不崩溃', function () {
    ...
    const processLine = {
      ...
      totalCycle: '7-8年',
      phases: [],
      stages: [{ stageId: 'phase1_evaluation', stageName: '资格评估', order: 1, status: 'in_progress', steps: [] }],
      status: 'active',
      progress: 0,
      currentStage: '资格评估',
      readyMaterials: 0,
    ...
    expect(function () {
      st.saveProcessLine(processLine);
    }).not.toThrow();

    const saved = st.getAllProcessLines();
