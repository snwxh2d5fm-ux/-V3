...
const onboardingPaths = require('../data/onboarding-paths');
const onboardingStorage = require('../utils/onboarding-storage');

// Mock wx
global.wx = {
  setStorageSync: function (k, v) {
    global.__storage = global.__storage || {};
    global.__storage[k] = v;
  },
  getStorageSync: function (k) {
    return (global.__storage || {})[k] || null;
  },
  removeStorageSync: function (k) {
    delete (global.__storage || {})[k];
  },
  showToast: function (opts) {
    global.__lastToast = opts;
  },
...
global.getApp = function () {
  return { globalData: { membershipLevel: 'free' } };
};
...
describe('流程控双通道逻辑 (TC-2.3.1~2.3.5)', function () {
  test('STAGE_BRIDGE_MAP 结构完整性', function () {
    const map = constants.STAGE_BRIDGE_MAP;
  ...
  test('ui_stages isMilestone 正确赋值', function () {
    const stages = constants.STAGE_BRIDGE_MAP.ui_stages;
  ...
  test('TC-2.3.1 通道A: phase1_evaluation → uiStage=0 → isMilestone=false', function () {
    const info = constants.STAGE_BRIDGE_MAP.phase_to_ui.phase1_evaluation;
  ...
  test('TC-2.3.2 通道B: phase2_onboarding → milestoneStageIndex=1(isMilestone=true，4子阶段)', function () {
    const info = constants.STAGE_BRIDGE_MAP.phase_to_ui.phase2_onboarding;
...
describe('攻略书关卡解锁逻辑 (TC-3.1.1~3.2.2)', function () {
  test('TC-3.1.1 FULL_PHASES 全部8关返回', function () {
    const result = onboardingPaths.assemblePath({
      visaType: 'qmas',
      familyStatus: 'single',
  ...
  test('TC-3.2.1 getGuideUnlockState: processStage=0 → 关卡3~7锁定, 0~2解锁', function () {
    const state = constants.STAGE_BRIDGE_MAP.getGuideUnlockState(0);
  ...
  test('TC-3.2.1 getGuideUnlockState: processStage=1(材料准备) → 关卡0~4解锁', function () {
    const state = constants.STAGE_BRIDGE_MAP.getGuideUnlockState(1);
  ...
  test('TC-3.2.1 getGuideUnlockState: processStage=4(获批激活) → 关卡0~6解锁', function () {
    const state = constants.STAGE_BRIDGE_MAP.getGuideUnlockState(4);
  ...
  test('TC-3.2.1 getGuideUnlockState: processStage=5(抵港生活) → 全部8关解锁', function () {
    const state = constants.STAGE_BRIDGE_MAP.getGuideUnlockState(5);
    expect(state[7]).toBe(true); // 5 >= 5 → true
  });

  test('TC-3.2.2 processStage 不会回退: 边界保护', function () {
    const state1 = constants.STAGE_BRIDGE_MAP.getGuideUnlockState(-1);
    const state2 = constants.STAGE_BRIDGE_MAP.getGuideUnlockState(99);
    // 钳制到0~6范围
...
describe('攻略书任务筛选回归 (REG-3.x)', function () {
  ...
  test('REG-3.x 时间预估映射正确', function () {
    const t = onboardingPaths.getPhaseTimeEstimate(0);
    expect(t).toContain('1-4周');
    const t3 = onboardingPaths.getPhaseTimeEstimate(3);
