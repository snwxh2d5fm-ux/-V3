...
Page({
  ...
  loadUserPath() {
    try {
      ...
      const userData = wx.getStorageSync(CONSTANTS.STORAGE_KEYS.USER_PROFILE) || {};
      // 优先取 globalData（实时，无存储延迟）
      let path =
        (app && app.globalData && app.globalData.selectedPath) || session.selectedPath || userData.selectedPath || '';
      ...
      const nameFromProcess =
        (app && app.globalData && app.globalData.activeProcess && app.globalData.activeProcess.name) || '';
      const pathNames = {
        qmas: '优才计划 (QMAS)',
        ttps_a: '高才通 A类',
        ttps_b: '高才通 B类',
  ...
  loadIdentity() {
    try {
      const profile = wx.getStorageSync(CONSTANTS.STORAGE_KEYS.IDENTITY_PROFILE) || {};
      const subStatus = wx.getStorageSync(CONSTANTS.STORAGE_KEYS.USER_SUB_STATUS) || '';
      let persona = 0;
      let personaLabel = '';
      if (subStatus.indexOf('student') > -1) {
        persona = 1;
        personaLabel = '在校学生';
      } else if (subStatus.indexOf('employed') > -1) {
        persona = 2;
        personaLabel = '在职人士';
      } else if (subStatus.indexOf('owner') > -1) {
        persona = 4;
        personaLabel = '企业主';
      } else if (subStatus.indexOf('overseas') > -1) {
        persona = 7;
        personaLabel = '海外华人';
      }
  ...
  loadDocuments() {
    try {
      const docs = getAllDocuments().filter(function (d) {
  ...
  buildChecklist(path) {
    ...
    // 注入身份适配的条件材料
    const withConditional = this.injectConditionalDocs(pathChecklists[path] || []);
  ...
  injectConditionalDocs(baseList) {
    ...
    if (identity.hasChildren) {
      for (let c = 1; c <= identity.childCount; c++) {
        conditionalDocs.push(
          {
            id: 'child_birth_cert_' + c,
            name: '子女' + c + '出生证明',
            category: '家属材料',
            required: true,
            stage: 1,
  ...
  matchChecklist() {
    ...
    const allDocs = this.data.allDocs || [];
    let hasCount = 0,
      missingCount = 0,
      optionalCount = 0;

    const matched = checklist.map(function (item) {
      const found = allDocs.find(function (d) {
      ...
      const docSpec = DOC_SPECS[item.id] || {};
      ...
      if (found && found.ocrData) {
        ...
        if (found.ocrData.idNumber && found.ocrData.idNumber !== '未识别') {
          let idStr = String(found.ocrData.idNumber);
          // 脱敏显示：只显示前2位+后4位
          if (idStr.length > 6) {
    ...
    // 构建优先级分区
    const totalRequired = checklist.filter(function (c) {
      return c.required;
    }).length;
    const progress = totalRequired > 0 ? Math.round((hasCount / totalRequired) * 100) : 0;
    ...
    // 移除空分区
    const priorityZones = zones.filter(function (z) {
      return z.items.length > 0;
    });

    // 初始化展开状态
    const showZoneDetail = {};
    priorityZones.forEach(function (z) {
      showZoneDetail[z.zone] = true;
    });
  ...
  toggleZone(e) {
    ...
    const show = this.data.showZoneDetail;
    show[zone] = !show[zone];
  ...
  onDocTap(e) {
    ...
    if (item.status === 'has' && item.docId) {
      wx.navigateTo({ url: '/subpkg-docs/pages/documents-detail/index?id=' + item.docId });
    } else {
      // 缺失/可选：弹出规格说明 + 快速添加
  ...
  // 规格弹窗
  showSpecForDoc(item) {
    const doc = DOC_SPECS[item.id] || { tip: '请按要求准备材料', validity: '未知', size: 'A4' };
  ...
  quickAddDoc() {
    this.hideSpecSheet();
    wx.navigateTo({ url: '/subpkg-docs/pages/documents-add/index' });
  },
  ...
  navigateToProfile() {
    ...
    try {
      ...
      const docs = storage.getAllDocuments ? storage.getAllDocuments() : [];
      const idDoc = docs.find(function (d) {
        return d.slotKey === 'id_card' || d.type === 'id_card';
      });
      ...
      // 2. 评估结果
      const assessment = wx.getStorageSync('__assessment_result__') || {};
      ...
      // 3. 攻略书进度
      const guideProgress = wx.getStorageSync('onboarding-progress') || {};
      let guideSummary = '';
      if (guideProgress.tasks) {
        const completed = Object.values(guideProgress.tasks).filter(function (t) {
          return t.status === 'completed';
        }).length;
        const total = Object.keys(guideProgress.tasks).length;
        guideSummary = '已完成 ' + completed + '/' + total + ' 项通关任务';
      }

      // 4. 身份状态
      const subStatus = wx.getStorageSync(CONSTANTS.STORAGE_KEYS.USER_SUB_STATUS) || '';
      const userStatus = app.globalData.userStatus || session.userStatus || '';
      const statusLabel = userStatus === 'approved' ? '已获批' : userStatus === 'unapplied' ? '未申请' : '';

      // 5. 路径选择
      const path = app.globalData.selectedPath || session.selectedPath || profile.selectedPath || '';
      const pathNames = {
        qmas: '优才',
        ttps_a: '高才A',
        ttps_b: '高才B',
  ...
  generateExportText() {
    const zones = this.data.priorityZones;
    const lines = [
      '📋 住港伴 · 智能材料清单',
      '路径: ' + this.data.pathName,
      '完成度: ' + this.data.progress + '%',
      '',
    ];
    zones.forEach(function (z) {
      lines.push('── ' + z.icon + ' ' + z.label + ' ──');
      z.items.forEach(function (item) {
        const icon = item.status === 'has' ? '✅' : item.status === 'missing' ? '⬜' : '⚪';
        const tag = item.required ? '[必需]' : '[建议]';
