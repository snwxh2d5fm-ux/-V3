...
Component({
  ...
  lifetimes: {
    attached() {
      const saved = wx.getStorageSync('__ai_conversation__');
  ...
  methods: {
    ...
    async sendMessage(e) {
      const text = ((e && e.detail && e.detail.value) || this.data.inputValue || '').trim();
      if (!text || this.data.loading) return;
      ...
      try {
        const pageCtx = this.properties.context || {};
        // P1-1: K2隐私 — pageContext仅传脱敏页面标识，不传用户可见文本
        const safePageContext = typeof pageCtx.pageContext === 'string' ? pageCtx.pageContext.substring(0, 80) : '';
        ...
        const res = await api.sendChatMessageV5(
    ...
    selectPathFromChat(pathType, label) {
      ...
      if (app.globalData.cloudReady && app.globalData.isLoggedIn) {
        try {
          ...
          const openid = cloudUser._openid || app.globalData.userInfo?._openid;
          if (openid) {
    ...
    parseMarkdownToNodes(text) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (/^[-*]\s+/.test(trimmed)) {
          ...
          nodes.push({
            ...
            children: [
              {
                name: 'span',
                attrs: { style: 'color:#3B82F6;margin-right:4px' },
        ...
        if (/^\d+[\.\、]\s*/.test(trimmed)) {
          ...
          const numText = trimmed.substring(numMatch[0].length);
          nodes.push({
            name: 'p',
            attrs: { style: 'padding-left:8px;margin:2px 0;font-size:15px;line-height:1.6' },
            children: [
              {
                name: 'span',
                attrs: { style: 'color:#3B82F6;margin-right:4px;font-weight:600' },
                children: [{ type: 'text', text: numMatch[1] + '.' }],
              },
    ...
    parseInlineMarkdown(text) {
      ...
      while ((match = boldRegex.exec(text)) !== null) {
        ...
        children.push({
          name: 'span',
          attrs: { style: 'font-weight:700;color:#1F2937' },
