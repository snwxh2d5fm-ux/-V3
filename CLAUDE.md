...
**核心规则：新增功能/格式化/重构时，不得删除或修改已有防御性代码。**

- `process.env.X || 'fallback'` 中的 `||` 兜底值**永远不得删除**（即使看起来像硬编码）
- 云函数 `switch(action)` 的现有 `case` 不得删除，只能新增
- 格式化 commit 只能改空格/换行/引号风格，不得改逻辑
- 删除任何代码行前，确认它在 commit message 中有说明
- `console.error` / `console.warn` 调用不得降级为 `console.debug`
- 详细规则见 `.hermes/rules/change-safety.md`

**最近教训**: 2026-05-22 `TOKEN_SECRET || 'zhgb-internal-key'` 被合并时意外删除 → 手机号登录崩溃
