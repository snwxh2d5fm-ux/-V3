...
module.exports = {
  ...
  CONFIDENCE_LEVELS: {
    ...
    D: {
      ...
      showBanner: true,
      bannerText: '以下内容基于合理推断，入境处有酌情权，建议个案咨询',
    },
    E: {
      label: 'E级·无法确认',
      color: '#9CA3AF',
  ...
  MEMBERSHIP_LIMITS: {
    ...
    basic: {
      ...
      maxProcessLines: Infinity,
      assessmentPerMonth: Infinity,
  ...
  getEffectiveLimit: function (membershipLevel, key) {
    const tier = membershipLevel && this.MEMBERSHIP_LIMITS[membershipLevel] ? membershipLevel : 'free';
    return this.MEMBERSHIP_LIMITS[tier][key];
  },

  /** 是否为付费会员（含 basic/pro/premium）。防御性：未知等级→非付费 */
  isPayingMember: function (membershipLevel) {
    const level = membershipLevel && this.MEMBERSHIP_LIMITS[membershipLevel] ? membershipLevel : 'free';
  ...
  STAGE_BRIDGE_MAP: {
    ...
    stageToUiStage: function (phaseId, uiStageIndex) {
      ...
      const info = this.phase_to_ui[phaseId];
    ...
    _validateBridgeMap: function () {
      ...
      for (let i = 0; i < this.ui_stages.length; i++) {
  ...
  ASSESSMENT_QUESTIONS: [
    ...
    },
    {
      ...
      examples: [
        '清华大学硕士+985本科 • 高才B✓（合资格学士）+ QMAS✓（硕士加分）',
        '非合资格大学本科+硕士 • 不能走高才B/C，可走优才QMAS',
        '高中毕业+资产充裕 • 走CIES或作为受养人',
      ],
    },
    {
      id: 'school',
      ...
      examples: [
        '清华大学学士+海外硕士 • 高才B✓（学士合资格），硕士另外加分',
        '双非本科+港大硕士 • 高才B✗（学士不合资格），优才✓',
        '港中文本科 • IANG✓ + 高才B✓（港校均在名单内）',
      ],
    ...
    },
    {
      ...
      examples: [
        '计算机博士+专利 • TechTAS+优才准则F1双通道',
        '金融专业+CFA+10年 • 优才人才清单+准则D1',
        '人文社科硕士 • 优才仅靠工作经验/语言等其他准则',
      ],
    },
    {
      id: 'industry',
      ...
      examples: [
        '金融科技5年 • 人才清单+准则D3触发',
        '传统制造业8年 • 走专才ASMTP（雇主担保）',
        '创科研发3年 • TechTAS优先+人才清单叠加',
      ],
    },
    {
      id: 'experience',
      ...
      examples: [
        '8年金融经验 • 优才QMAS准则D1✓ + 高才B✓',
        '< 3年+合资格学士 • 高才C（配额制，先到先得）',
        '应届+港校毕业 • IANG✓，工作后再转优才',
      ],
    },
    {
      id: 'position',
    ...
    },
    {
      ...
      examples: [
        '年收入HK$280万 • 高才A✓，优才准则E✓',
        '年收入HK$80万+名校硕士 • 优才为主（准则E不触发），高才A✗',
        '年收入HK$25万 • 优才/专才为主，暂不符合高才A',
      ],
    },
    {
      id: 'company',
      ...
      examples: [
        '香港雇主担保 • 直接走专才ASMTP',
        '自雇+在港注册公司+运营2年 • 自雇专才/高才A续签',
        '自由职业+高收入 • 高才A（达标收入即可，不需雇主）',
      ],
    },
    {
      id: 'language',
    ...
    },
    {
      ...
      examples: [
        '已婚+1个未成年子女 • 全家同步申请受养人',
        '单身 • 仅本人申请，未来可添加',
        '主申永居后+父母60岁+独生 • 可申请父母赴港',
      ],
    },
    {
      id: 'hasIP',
