...
function matchPersonaToPaths(profile) {
  ...
  // 画像1: 名校毕业生 → 学生→IANG
  if (profile.age <= 28 && profile.education === '本科' && profile.eligibleSchool === true && profile.experience <= 3) {
  ...
  const hasChinese = profile.language && (profile.language.includes('中文') || profile.language.includes('粤语'));
  const hasEnglishFluent = profile.language && profile.language.includes('英语流利');
  ...
  if (qmasCriteria >= 6) {
    ...
    const qmasScore = Math.min(100, 50 + (qmasCriteria - 6) * 10);
    ...
    phases: [
      {
        ...
        duration: '3-6月',
        actions: ['获学校offer', '启动学生签证(6-8周)', '办理港澳通行证+逗留D签注', '经济准备(首年30-40万)'],
        confidence: 'B',
      },
      {
        phase: 'phase2_onboarding',
        duration: '0-6月',
        actions: ['入境激活e-Visa(3月内)', '办理香港身份证(30天)', '银行开户+租房+注册'],
        confidence: 'A',
      },
      {
        phase: 'phase3_maintenance',
        duration: '1-6年',
        actions: ['全日制学习', '申请IANG', 'IANG就业/创业', '续签(2-3年/次)'],
        confidence: 'A',
      },
      {
        phase: 'phase4_pr_sprint',
        duration: '第6-7年',
        actions: ['整理7年记录', '准备解释信(如长离港)', '提交永居核实申请'],
        confidence: 'A',
      },
    ],
    decisionPoints: [
    ...
    phases: [
      {
        phase: 'phase1_evaluation',
        duration: '1-2周',
        actions: ['核实年收入≥250万港币', '准备收入证明材料'],
        confidence: 'A',
      },
      {
        phase: 'phase2_onboarding',
        duration: '0-3月',
        actions: ['在线递交申请(审批4周)', '下载e-Visa', '携e-Visa入境激活'],
        confidence: 'A',
      },
      {
        phase: 'phase3_maintenance',
        duration: '1-6年',
        actions: ['36月首次签证', '续签(就业/创业)', 'MPF+税单积累'],
        confidence: 'B',
      },
      {
        phase: 'phase4_pr_sprint',
        duration: '第6-7年',
        actions: ['提前6月准备续签材料', '整理在港记录', '递交永居申请'],
        confidence: 'A',
      },
    ],
    ...
    phases: [
      {
        phase: 'phase1_evaluation',
        duration: '1-2周',
        actions: ['确认学士为合资格大学', '确认≥3年工作经验'],
        confidence: 'A',
      },
      {
        phase: 'phase2_onboarding',
        duration: '0-2月',
        actions: ['在线递交申请(审批4周)', '下载e-Visa', '赴港激活'],
        confidence: 'A',
      },
      {
        phase: 'phase3_maintenance',
        duration: '1-6年',
        actions: ['24月首次签证', '就业或创业', '续签'],
        confidence: 'B',
      },
    ...
    phases: [
      {
        phase: 'phase1_evaluation',
        duration: '1-2周',
        actions: ['确认学士为合资格大学', '确认毕业≤5年', '确认配额可用(年度10000名)'],
        confidence: 'A',
      },
      {
        phase: 'phase2_onboarding',
        duration: '0-2月',
        actions: ['在线递交申请', '下载e-Visa', '赴港激活'],
        confidence: 'A',
      },
      {
        phase: 'phase3_maintenance',
    ...
    phases: [
      {
        phase: 'phase1_evaluation',
        duration: '1-3月',
        actions: ['12项准则自评(≥6项)', '准备赴港计划书', '收集学历+工作+资产证明'],
        confidence: 'B',
      },
      {
        phase: 'phase2_onboarding',
        duration: '0-6月',
        actions: ['递交申请(审批6-12月)', '获批24月签证', '携e-Visa入境激活'],
        confidence: 'B',
      },
      {
        phase: 'phase3_maintenance',
        duration: '1-6年',
        actions: ['2+3+3续签模式', '证明「通常居住」', 'MPF+税单'],
        confidence: 'C',
      },
      {
        phase: 'phase4_pr_sprint',
        duration: '第6-7年',
        actions: ['证明在港「通常居住」7年', '永居申请'],
        confidence: 'B',
      },
    ],
    decisionPoints: [
...
const DECISION_COMPARISONS = {
  dp2_student_to_work: {
    options: [
      ...
      { option: 'B', label: '优才', passRate: '~70%', cycle: '3+3+2', risk: '中', note: '12项准则≥6项，6-12月审批' },
