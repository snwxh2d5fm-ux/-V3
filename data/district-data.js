...
function getCommute(d, workRegionId) {
  switch (workRegionId) {
    case 'hong-kong-island':
      return d.commute.central;
    case 'kowloon':
      return d.commute.tst;
...
module.exports.matchDistricts = function (budget, workRegion, hasChildren) {
  const districts = module.exports;
  const brackets = module.exports.BUDGET_BRACKETS;

  // 1. 确定用户预算档位
  let userBracket = null;
  for (var i = 0; i < brackets.length; i++) {
    if (budget >= brackets[i].min && budget <= brackets[i].max) {
      userBracket = brackets[i];
      break;
    }
  }
  if (!userBracket) {
  ...
  for (i = 0; i < districts.length; i++) {
    ...
    const min2Bed = parseMinRent(d.priceRange.twoBed || d.priceRange.studio);
  ...
  for (i = 0; i < affordable.length; i++) {
    ...
    const commute = getCommute(d, workRegion);
    ...
    const WORK_REGION_MAP = { 'hong-kong-island': '港岛', kowloon: '九龙', 'new-territories': '新界' };
    const userRegion = WORK_REGION_MAP[workRegion] || '';
    ...
    const zoneBracket = d.avgRentPsf < 30 ? 0 : d.avgRentPsf < 40 ? 1 : d.avgRentPsf < 50 ? 2 : 3;
    const userBracketIdx = brackets.indexOf(userBracket);
