...
exports.main = async (event, context) => {
  ...
  try {
    switch (action) {
      case 'start':            return await startProcess(openid, event);
      case 'getTemplates':     return await getTemplates(event);
      case 'getTemplate':      return await getTemplate(event.templateId);
      case 'getActive':        return await getActive(openid, event);
      case 'getMyProcess':     return await getMyProcess(openid, event.processId);
      case 'getMyProcesses':   return await getMyProcesses(openid);
      case 'getProgress':      return await getProgress(openid, event.processId);
...
async function getTemplates(event) {
  ...
  const result = await db.collection('process_templates')
    .where(query).orderBy('templateId', 'asc').get();
...
async function startProcess(openid, event) {
  ...
  // 检查是否已有进行中的同模板流程
  const existing = await db.collection('user_processes')
    .where({ _openid: openid, templateId, status: 'active' }).get();
  ...
  const stages = tmpl.stages.map((s, i) => ({
    ...
    steps: s.steps.map(st => ({
      ...
      completedAt: null, materials: [], notes: null
    }))
  }));
  ...
  const process = {
    ...
    overallProgress: 0,
    stages, milestones, exceptions: [], history: [],
    status: 'active',
  ...
  await _logAudit(openid, 'process_start', {
    processId: result._id, templateId, templateName: tmpl.templateName
  });
...
async function getMyProcess(openid, processId) {
  const result = await db.collection('user_processes')
    .where({ _id: processId, _openid: openid }).get();
...
async function getMyProcesses(openid) {
  ...
  return {
    ...
    data: result.data.map(p => ({
      processId: p._id, templateId: p.templateId, status: p.status,
      currentStageId: p.currentStageId, overallProgress: p.overallProgress,
...
async function getProgress(openid, processId) {
  ...
  const template = await db.collection('process_templates')
    .where({ templateId: proc.templateId }).get();
  const tmpl = template.data[0];
  ...
  const stageProgress = (proc.stages || []).map(ps => {
    ...
    return {
      stageId: ps.stageId, stageName: ps.stageName,
      status: ps.status, order: ps.order,
  ...
  return {
    code: 0,
    data: {
      processId: proc._id, templateId: proc.templateId,
...
async function verifyMilestone(openid, event) {
  ...
  const proc = await db.collection('user_processes')
    .where({ _id: processId, _openid: openid }).get();
  ...
  // 获取模板中的OCR验证规则
  const template = await db.collection('process_templates')
    .where({ templateId: record.templateId }).get();
  ...
  // 验证通过
  await _updateMilestone(processId, stageId, docId, {
    attempts, isMatch: true, status: 'verified',
    ocrResult: { dateField: ocrResult?.dateField, numberField: ocrResult?.numberField },
    verifiedAt: db.serverDate()
  });
  ...
  await db.collection('milestone_records').add({
    data: {
      ...
      expectedDocType, actualDocType, isMatch: true, confidence: ocrResult?.confidence || 1,
      attempts, isForwardJump, unlockedStageIds,
      ocrResult: { dateField: ocrResult?.dateField || null, numberField: ocrResult?.numberField || null },
      verifiedAt: db.serverDate(), createdAt: db.serverDate()
    }
  });
  ...
  return {
    code: 0,
    data: {
      verified: true, unlockedStageIds, isForwardJump,
      nextStageId,
      currentStageId: nextStageId
    }
  };
...
async function getChecklist(openid, templateId, userDocIds) {
  ...
  const tmpl = template.data[0];
  const userDocs = await db.collection('user_documents')
    .where({ _id: _.in(userDocIds || []), _openid: openid }).get();
    ...
    for (const step of stage.steps || []) {
      for (const mat of step.requiredMaterials || []) {
        materialMap[mat.materialType] = {
          materialType: mat.materialType, isOptional: mat.isOptional,
          condition: mat.condition, matchedDocs: []
        };
      }
    }
  }
  ...
  for (const doc of userDocs.data) {
    if (materialMap[doc.docType]) {
      materialMap[doc.docType].matchedDocs.push({
        docId: doc._id, docName: doc.docName,
  ...
  await db.collection('user_processes').where({ _id: processId }).update({
    data: {
      exceptions: _.push({
        ...
        description, resolvedAt: null, resolution: null
      }),
      updatedAt: db.serverDate()
    }
  });
