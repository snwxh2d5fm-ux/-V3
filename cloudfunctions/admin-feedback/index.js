...
exports.main = async (event) => {
  ...
  if (!(await db.collection('admin_users').where({ apiKeyHash: sha256(_apiKey), status: 'active' }).limit(1).get()).data.length) return { code: 401, msg: '无效 API Key' };

  try {
    switch (action) {
      case 'listFeedback': return listFeedback(params);
      case 'getFeedbackDetail': return feedbackDetail(params);
      case 'updateStatus': return updateStatus(params);
      case 'getStats': return feedbackStats(params);
...
async function listFeedback(p) {
  ...
  const q = {}; if (status) q.status = status; if (type) q.type = type;
  const [list, cnt] = await Promise.all([
    db.collection('feedback').where(q).orderBy('createdAt','desc').skip((page-1)*pageSize).limit(pageSize).get(),
...
async function feedbackDetail(p) {
  ...
  const f = r.data[0]; f.content = sanitize(f.content||''); // 二次脱敏
  return { code: 0, data: f };
}

async function updateStatus(p) {
  await db.collection('feedback').where({ ticketId: p.ticketId }).update({ status: p.status, updatedAt: new Date().toISOString() });
...
async function feedbackStats(p) {
  ...
  return { code: 0, data: { total: total.total, resolved: resolved.total, closureRate: total.total ? Math.round(resolved.total/total.total*100)+'%' : '0%' } };
}
