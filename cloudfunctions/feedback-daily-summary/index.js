...
exports.main = async (event, context) => {
  ...
  try {
    switch (action) {
      case 'dailySummary':   return await dailySummary();
      case 'manualTrigger':  return await dailySummary();   // 手动触发，行为一致
      case 'getReport':      return await getReport(event);
      case 'listReports':    return await listReports(event);
      default:               return await dailySummary();
    }
  } catch (err) {
...
async function dailySummary() {
  ...
  console.log('[daily-summary] 生成报告:', reportDate,
    'range:', new Date(rangeStart).toISOString(), '~', new Date(rangeEnd).toISOString());
  ...
  try {
    ...
    console.log('[daily-summary] 报告已存储:', reportId);
  } catch (dbErr) {
...
async function sendEmail(reportDate, report, reportId) {
  ...
  if (!EMAIL_API_URL || !EMAIL_API_KEY) {
    console.log('[daily-summary] 未配置 EMAIL_API_URL，跳过邮件发送。报告ID:', reportId);
  ...
  return new Promise((resolve, reject) => {
    const req = transport.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          console.log('[daily-summary] 邮件发送成功:', res.statusCode);
    ...
    req.on('timeout', () => { req.destroy(); reject(new Error('邮件请求超时')); });
    req.write(postData);
    req.end();
  });
}
...
function buildEmailHtml(reportDate, report, reportId) {
  ...
  var inv = report.invoices;

  // ★ 复用祖脉开票字段：invoiceType → 个人/企业，title → 抬头
  var invoiceRows = inv.items.map(function(item, i) {
    var typeLabel = item.invoiceType === 'company' ? '🏢 企业' : '🧑 个人';
    var statusLabel = { pending: '⏳处理中', issued: '✅已开具', rejected: '❌已退回' }[item.status] || item.status;
    return '<tr>' +
      '<td>' + (i + 1) + '</td>' +
      '<td>' + escapeHtml(item.orderId || '').substring(0, 12) + '</td>' +
      '<td>' + escapeHtml(item.productName || '') + '</td>' +
      '<td>' + escapeHtml(item.title || '') + '</td>' +
      '<td>' + typeLabel + '</td>' +
      '<td>¥' + (item.orderAmountYuan || '0.00') + '</td>' +
      '<td>' + statusLabel + '</td>' +
      '<td>' + escapeHtml(item.email || '') + '</td>' +
      '</tr>';
  }).join('');

  var feedbackRows = f.items.map(function(item, i) {
    var typeLabel = { bug: '🐛功能异常', content: '📝内容错误', other: '💬其他' }[item.type] || item.type;
    var statusLabel = { submitted: '已提交', in_progress: '处理中', replied: '已回复', closed: '已关闭' }[item.status] || item.status;
    return '<tr>' +
      '<td>' + (i + 1) + '</td>' +
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
    ...
    '<h1>住港伴 · 每日运营日报</h1>' +
    '<p class="date">报告日期：' + reportDate + ' | 报告ID：' + (reportId || '-') + '</p>' +

    // ★ 总览
    '<h2>📊 总览</h2>' +
    '<div class="stat-row">' +
    '<div class="stat-box"><div class="stat-num">' + f.total + '</div><div class="stat-label">反馈工单</div></div>' +
    '<div class="stat-box"><div class="stat-num">' + inv.total + '</div><div class="stat-label">开票申请</div></div>' +
    '<div class="stat-box"><div class="stat-num">¥' + (inv.totalAmountYuan || '0.00') + '</div><div class="stat-label">开票金额</div></div>' +
    '</div>' +

    // ★ 反馈工单
    '<h2>💬 反馈工单 (' + f.total + '条)</h2>' +
    (f.total > 0 ?
      '<table><thead><tr><th>#</th><th>工单号</th><th>类型</th><th>内容</th><th>状态</th><th>匿名</th><th>附件</th></tr></thead><tbody>' +
      feedbackRows + '</tbody></table>' :
      '<p style="color:#999">今日无反馈工单</p>') +

    // 反馈分布
    (f.total > 0 ?
      '<div class="stat-row">' +
      '<div class="stat-box"><div class="stat-num">' + (f.byType.bug || 0) + '</div><div class="stat-label">🐛 功能异常</div></div>' +
      '<div class="stat-box"><div class="stat-num">' + (f.byType.content || 0) + '</div><div class="stat-label">📝 内容错误</div></div>' +
      '<div class="stat-box"><div class="stat-num">' + (f.byType.other || 0) + '</div><div class="stat-label">💬 其他</div></div>' +
      '<div class="stat-row">' +
      '<div class="stat-box"><div class="stat-num">' + (f.byStatus.submitted || 0) + '</div><div class="stat-label">待处理</div></div>' +
      '<div class="stat-box"><div class="stat-num">' + (f.byStatus.in_progress || 0) + '</div><div class="stat-label">处理中</div></div>' +
      '<div class="stat-box"><div class="stat-num">' + (f.byStatus.replied || 0) + '</div><div class="stat-label">已回复</div></div>' +
      '</div>' : '') +

    // ★ 开票记录
    '<h2>🧾 开票记录 (' + inv.total + '条)</h2>' +
    (inv.total > 0 ?
      '<table><thead><tr><th>#</th><th>订单号</th><th>商品</th><th>抬头</th><th>类型</th><th>金额</th><th>状态</th><th>邮箱</th></tr></thead><tbody>' +
      invoiceRows + '</tbody></table>' :
      '<p style="color:#999">今日无开票申请</p>') +

    '<div class="stat-row">' +
    '<div class="stat-box"><div class="stat-num">' + (inv.byStatus.pending || 0) + '</div><div class="stat-label">⏳ 待处理</div></div>' +
    '<div class="stat-box"><div class="stat-num">' + (inv.byStatus.issued || 0) + '</div><div class="stat-label">✅ 已开具</div></div>' +
    '<div class="stat-box"><div class="stat-num">' + (inv.byStatus.rejected || 0) + '</div><div class="stat-label">❌ 已退回</div></div>' +
    '</div>' +
...
async function getReport(event) {
  ...
  var reportDate = event.reportDate;

  var query = { type: 'feedback_invoice_daily' };
  if (reportId) {
    try {
  ...
  if (reportDate) {
    var result = await db.collection('daily_reports')
...
async function listReports(event) {
  var limit = Math.min(event.limit || 30, 90);
