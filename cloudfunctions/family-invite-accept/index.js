...
exports.main = async (event) => {
  try {
    ...
    if (now > expiresAt) {
      await db.collection('family_invites').where({
        inviteCode: inviteCode
      }).update({
        data: {
          status: 'expired',
    ...
    // 检查接受者是否已在某个家庭空间中
    var existingMemberSpaces = await db.collection('family_spaces')
      .where(_.or([
        { ownerUserId: openid },
    ...
    // 原子更新邀请状态
    var updateResult = await db.collection('family_invites').where({
      inviteCode: inviteCode,
      status: 'pending'
    }).update({
      data: {
