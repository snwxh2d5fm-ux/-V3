...
exports.main = async (event) => {
  ...
  try {
    switch (action) {
      case 'getArticleRanking': return articleRanking();
      case 'getTaskCompletion': return taskCompletion();
      case 'getSearchHotwords': return searchHotwords();
...
async function articleRanking() {
  ...
  return { code: 0, data: (articles.data||[]).map(a => ({ title: a.title, category: a.category, createdAt: a.createdAt })) };
}

async function taskCompletion() {
  const tasks = await db.collection('life_guide_tasks').limit(100).get();
  const byCat = {}; (tasks.data||[]).forEach(t => { const c = t.category||'other'; if (!byCat[c]) byCat[c] = { total: 0, completed: 0 }; byCat[c].total++; if (t.completed) byCat[c].completed++; });
...
async function searchHotwords() {
  ...
  const words = {}; (logs.data||[]).forEach(l => { const q = (l.query||'').slice(0,30); if (q) words[q] = (words[q]||0)+1; });
  const top = Object.entries(words).sort((a,b) => b[1]-a[1]).slice(0,20).map(([w,c]) => ({ word: w, count: c }));
