...
const PERSONA_FEATURES = {
  ...
  6: {
    ...
    eligibleSchool: true,
    expMax: 5,
    corePaths: ['ttps_b', 'ttps_c', 'student_iang'],
  },
...
const PATH_PROFILES = {
  student_iang: {
    ...
    passRate: 0.92,
    confidence: 'B',
    requirements: ['获港校offer', '全日制学习', '毕业申请IANG', '在港就业'],
  },
  parttime_qmas: {
    name: '兼读制→优才/专才→永居',
    ...
    passRate: 0.65,
    confidence: 'C',
    requirements: ['毕业后另寻路径', '优才12准则≥6项或专才雇主'],
    notes: ['兼读制不申IANG', '旅游签入境上课属法律灰色地带'],
  },
  ttps_a: {
    name: '高才通A类(≥250万)',
    riskLevel: 'low',
    ...
    passRate: 0.95,
    confidence: 'A',
    requirements: ['年收入≥250万港币', '纳税/审计证明'],
    firstVisa: '36月',
  },
  ttps_b: {
    name: '高才通B类(学士+3年)',
  ...
  ttps_c: {
    ...
    passRate: 0.9,
    confidence: 'B',
    requirements: ['合资格大学学士', '毕业≤5年', '配额可用'],
    firstVisa: '24月',
    notes: ['年度配额10000名'],
  },
  qmas: {
    ...
    passRate: 0.7,
    confidence: 'C',
    requirements: ['12项准则≥6项', '赴港计划书'],
    notes: ['"通常居住"无硬性天数要求，建议≥180天/年'],
  },
  asmpt: {
    name: '专才ASMTP(雇主sponsor)',
    riskLevel: 'medium',
    totalCycle: '7年',
    passRate: 0.85,
    confidence: 'B',
    requirements: ['香港雇主sponsor', '证明职位无法本地填补'],
    notes: ['绑雇主', '换雇主需重新申请'],
  },
  techtas: {
    name: '科技人才TechTAS',
  ...
  cies: {
    ...
    passRate: 0.95,
    confidence: 'A',
    requirements: ['准资产≥3000万', '持续撷持有效资产'],
  },
};
...
exports.main = async (event) => {
  ...
  try {
    switch (action) {
      case 'match':
        return await matchPaths(event.profile);
      case 'compare':
        return await comparePaths(event.pathIds);
      case 'getDetail':
        return await getPathDetail(event.pathId);
      case 'listAll':
...
async function matchPaths(profile) {
  ...
  if (p.age >= 28 && p.age <= 40 && p.experience >= 3 && (p.income || 0) < 2500000)
    scores.qmas = (scores.qmas || 0) + 40;
  ...
  if (p.major === 'STEM' && p.experience >= 2) {
    scores.techtas = (scores.techtas || 0) + 80;
    scores.qmas = (scores.qmas || 0) + 30;
  }
  if (p.capital >= 30000000) scores.cies = (scores.cies || 0) + 100;
  if (p.companyType === 'enterprise_owner' && p.income >= 2500000) {
    scores.ttps_a = (scores.ttps_a || 0) + 85;
    scores.cies = (scores.cies || 0) + 60;
  }
...
function desensitizeProfile(profile) {
  ...
  for (const k of keep) {
    if (profile[k] !== undefined) safe[k] = profile[k];
  }
  return safe;
}
