...
async function redeemCode(openid, { code, deviceId }) {
  ...
  if (deviceId) {
    ...
    const { data: deviceRecords } = await db
...
async function getMembership(openid) {
  ...
  const daysRemaining = isActive ? Math.ceil((expireAt - now) / 86400000) : 0;
...
async function generateSeedCodes({ count, channel, expiresInDays, batchId }) {
  ...
  while (codes.length < genCount && tried < attempts) {
    ...
    const { data: existing } = await db.collection(CODE_COLLECTION).where({ code: newCode }).limit(1).get();
...
async function resetTestUser(openid) {
  ...
  const { data: subs } = await db.collection('subscription_records').where({ _openid: openid, status: 'active' }).get();
  for (const s of subs) {
    await db
      .collection('subscription_records')
      .doc(s._id)
...
async function activateMembership(openid, inviteCode) {
  ...
  if (existingSubs.data.length > 0) {
    ...
    const newExpire = currentExpire > now ? new Date(currentExpire.getTime() + MEMBERSHIP_DAYS * 86400000) : expireAt;
    await db
      .collection('subscription_records')
      .doc(sub._id)
      .update({
