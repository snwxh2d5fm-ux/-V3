...
function scoreDomain(queryLower, keywords) {
  ...
  // 精确匹配（完整短语）
  for (let i = 0; i < keywords.exact.length; i++) {
  ...
  // 同义词匹配
  for (let j = 0; j < keywords.synonyms.length; j++) {
  ...
  // 子串匹配（短词，容易命中，权重低）
  for (let k = 0; k < keywords.partials.length; k++) {
...
const GENERIC_WORDS = ['你好', '谢谢', '帮助', '能做什么', '功能', '介绍一下'];
...
function detectDomain(message, mode) {
  ...
  for (let d = 0; d < domains.length; d++) {
    ...
    const score = scoreDomain(lower, DOMAIN_KEYWORDS[domain]);
    if (score > 0) {
  ...
  if (scored.length === 0) {
    for (let g = 0; g < GENERIC_WORDS.length; g++) {
  ...
  if (mode === 'qa') {
    ...
    if (scored.length > 0 && scored[0].score >= 10) {
      const isClear = scored.length === 1 || scored[0].score - scored[1].score >= 5;
      if (isClear) {
        return scored[0].domain;
      }
    }
