...
function scoreQMAS(answers) {
  ...
  const stemFields = ['STEM', '科学', '科技', '工程', '数学', '技术'];
  const isStem = stemFields.some(function (f) {
    return major.indexOf(f) > -1;
  });
  if (isStem) {
    criteriaMet++;
  ...
  const hasEnglish = lang.indexOf('英语') > -1;
  const hasChinese = lang.indexOf('中文') > -1 || lang.indexOf('母语') > -1;
  // 选项如"英语（流利）"默认中文母语用户，视为双语
  const hasTwoLangs = (hasEnglish && hasChinese) || lang.indexOf('英语') > -1;
  if (hasTwoLangs) {
    criteriaMet++;
  ...
  // C2. 语言：英文是否达标（流利=达标）
  const isEnglishGood =
    lang.indexOf('流利') > -1 || lang.indexOf('雅思') > -1 || lang.indexOf('托福') > -1 || lang.indexOf('六级') > -1;
  if (isEnglishGood) {
    criteriaMet++;
  ...
  const isTopCompany =
    company.indexOf('500强') > -1 ||
    company.indexOf('上市') > -1 ||
    company.indexOf('知名') > -1 ||
    company.indexOf('龙头') > -1;
  ...
  const isTargetIndustry = ['金融', '会计', '资讯科技', '科技', '工程', '贸易'].some(function (i) {
  ...
  // D4. 工作：是否2年以上国际经验
  const hasIntlExp = company.indexOf('海外') > -1 || industry.indexOf('海外') > -1 || lang.indexOf('粤语') > -1;
  ...
  const isLowIncome =
    income.indexOf('低于') > -1 ||
    income.indexOf('30万') > -1 ||
    income.indexOf('30-50') > -1 ||
    income.indexOf('50-100') > -1;
  const isHighIncome =
    income.indexOf('100-250') > -1 ||
    income.indexOf('250万') > -1 ||
    income.indexOf('200万') > -1 ||
    income.indexOf('300万') > -1 ||
  ...
  const isBizOwner = company.indexOf('创业') > -1 || company.indexOf('自雇') > -1;
  ...
  const probability = Math.min(95, Math.max(15, Math.round((criteriaMet / maxCriteria) * 100)));
  const isQualified = criteriaMet >= 6;
  ...
  return {
    ...
    details: details,
    summary: isQualified
      ? '满足申请资格（' + criteriaMet + '/12项准则）'
      : '暂不满足（' + criteriaMet + '/12项准则，需≥6项）',
...
function scoreTTPS(answers) {
  ...
  // A类：年收入≥250万港币
  const isHighIncome = income.indexOf('250万') > -1 || income.indexOf('300万') > -1 || income.indexOf('500万') > -1;
  if (isHighIncome) {
    candidates.push('A类');
  }
  ...
  const isQualifiedUni = school.indexOf('QS') > -1 || school.indexOf('百强') > -1;
  const hasBachelors = edu.indexOf('本科') > -1 || edu.indexOf('学士') > -1;
  if (isQualifiedUni && hasBachelors) {
    // 注意 "1-3年" 含 "3年" 子串，需先排除
    const isShortExp = exp.indexOf('1-3') > -1 || exp.indexOf('1年') > -1 || exp.indexOf('以下') > -1;
    const has3yrExp = exp.indexOf('3-5') > -1 || exp.indexOf('5-10') > -1 || exp.indexOf('10年') > -1;
  ...
  const classCount = candidates.length;
  const probability = classCount > 0 ? Math.min(95, 75 + classCount * 10) : 20;
  ...
  return {
    ...
    details: candidates.length > 0 ? ['符合条件：' + candidates.join('/')] : ['未达到A/B/C类硬性门槛'],
    summary: candidates.length > 0 ? '符合' + candidates.join('/') + '条件' : '暂不符合高才通条件',
...
function scoreASMTP(answers) {
  ...
  const inDemandIndustries = ['金融', '资讯科技', '科技', '工程', '医疗', '法律'];
  if (
    inDemandIndustries.some(function (i) {
      return industry.indexOf(i) > -1;
    })
  ) {
    score++;
...
function scoreIANG(answers) {
  ...
  const isHKGrad = school.indexOf('香港') > -1;
  const hasDegree = edu.indexOf('本科') > -1 || edu.indexOf('硕士') > -1 || edu.indexOf('博士') > -1;
...
function scoreInvestment(answers) {
  ...
  if (income.indexOf('250万') > -1) {
    score++;
  }

  const probability = 30 + score * 20;
