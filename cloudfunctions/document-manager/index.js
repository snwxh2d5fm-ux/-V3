...
exports.main = async (event) => {
  ...
  switch (action) {
    case 'list':
      return listDocs(OPENID);
    case 'save':
      return saveDoc(OPENID, data);
    case 'delete':
      return deleteDoc(OPENID, data);
    case 'sync':
      return syncDocs(OPENID, data);
    default:
...
async function deleteDoc(openid, { docId }) {
  // 验证文档归属——仅允许删除本人的文档
  const result = await db.collection('user_documents').where({ _id: docId, _openid: openid }).get();
...
async function syncDocs(openid, { localDocs }) {
  ...
  const { data: cloudDocs } = await db.collection('user_documents').where({ _openid: openid }).get();
