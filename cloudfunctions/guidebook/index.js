...
const CONFIDENCE_BADGE = {
  ...
  D: {
    ...
    color: '#DC2626',
    bg: '#FEF2F2',
    banner: '⚠️ 以下内容基于合理推断，入境处有酌情权，建议个案咨询',
  },
...
exports.main = async (event) => {
  ...
  try {
    switch (action) {
      ...
      case 'rateArticle':
        return await rateArticle(event);
      case 'getRecommended':
        return await getRecommended(event);
      case 'search':
        return await searchArticles(event);
      case 'getHotTags':
        return await getHotTags();
      default:
        return {
          code: 400,
          msg: '无效操作，支持: getArticles/getArticleDetail/rateArticle/getRecommended/search/getHotTags',
        };
    }
  } catch (err) {
...
async function getArticles(event) {
  ...
  try {
    ...
    console.debug('[guidebook] getArticles DB失败，返回空:', e.message);
...
async function getRecommended(event) {
  ...
  try {
    ...
    const result = await db.collection('guidebook_articles').orderBy('usefulCount', 'desc').limit(200).get();

    // ====== 评分函数（双重驱动） ======
    function scoreArticle(article) {
      let score = 0;
      const cat = DOMAIN_MAP[article.knowledgeDomain] || 'life';
      const tags = article.topics || [];

      // 第一驱动力：身份路径匹配（+20分，精准命中）
      if (pathTags.length > 0) {
        for (let p = 0; p < pathTags.length; p++) {
          if (tags.indexOf(pathTags[p]) >= 0) {
            score += 20;
            break;
          }
        }
    ...
    // 评分排序
    const scored = result.data.map(function (a) {
    ...
    for (let i = 0; i < scored.length; i++) {
      ...
      const cat = DOMAIN_MAP[scored[i].article.knowledgeDomain] || 'life';
    ...
    console.debug('[guidebook] V3 getRecommended失败:', e.message);
...
function enrichArticle(article) {
  ...
  if (!sections.length) {
    const body = article.mergedContent || article.content || '';
    if (body) {
