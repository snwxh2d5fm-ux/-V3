...
Page({
  ...
  loadOrders: function () {
    ...
    wx.cloud
      ...
      .then(function (res) {
        ...
        let orders = (result.data || []).map(function (o) {
          return {
            orderId: o.orderId,
        ...
        const deleted = wx.getStorageSync('__deleted_orders__') || [];
        if (deleted.length > 0) {
          orders = orders.filter(function (o) {
            return deleted.indexOf(o.orderId) < 0;
          });
        }
  ...
  viewDetail: function (e) {
    ...
    wx.navigateTo({ url: '/subpkg-chat/pages/orders-detail/index?orderId=' + orderId });
  },
  ...
  deleteOrder: function (e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          // 先从本地UI移除
          const orders = that.data.orders.filter(function (o) {
            return o.orderId !== orderId;
          });
          that.setData({ orders: orders, empty: orders.length === 0 });
          // 同步删除本地缓存
          let local = wx.getStorageSync('__user_orders__') || [];
