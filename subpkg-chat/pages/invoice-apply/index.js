...
Page({
  ...
  checkExistingInvoice: function () {
    ...
    wx.cloud
      ...
      .then(function (res) {
        ...
        const found = data.filter(function (inv) {
        ...
        if (found.length > 0) {
          ...
          wx.showModal({
            title: '已有发票申请',
            content:
              '该订单已申请过发票（状态：' + (inv.status === 'issued' ? '已开具' : '处理中') + '），无需重复申请',
            showCancel: false,
            success: function () {
              wx.navigateBack();
            },
  ...
  submitInvoice: function () {
    ...
    const invoiceType = that.data.invoiceType;
          ...
          if (result.code === 400 && result.msg && result.msg.indexOf('已申请过发票') >= 0) {
            wx.showModal({
              ...
              success: function (modalRes) {
                if (modalRes.confirm) {
                  wx.redirectTo({ url: '/subpkg-chat/pages/invoice-list/index' });
                }
              },
            });
          } else {
